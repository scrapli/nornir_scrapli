...
def test_send_configs_from_file(nornir, monkeypatch):
    ...
    result = nornir.run(task=send_configs_from_file, dry_run=False, file=CONFIG_FILE,)
...
def test_send_configs_from_file_dry_run(nornir, monkeypatch):
    ...
    result = nornir.run(task=send_configs_from_file, dry_run=True, file=CONFIG_FILE,)
...
def test_send_configs_from_file_generic_driver(nornir_generic, monkeypatch):
    ...
    result = nornir_generic.run(task=send_configs_from_file, dry_run=True, file="whatever",)
    assert (
